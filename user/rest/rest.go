...
func (m *manager) fetchExternalUserByEmail(ctx context.Context, email string) (*userpb.User, error) {
	...
	log := appctx.GetLogger(ctx)
	url := fmt.Sprintf("%s/api/v1.0/Identity/by_email/%s?filter=unconfirmed%%3Afalse&filter=blocked%%3Afalse&filter=disabled%%3Afalse&field=upn&field=primaryAccountEmail&field=displayName&field=type&field=source&field=activeUser", m.conf.APIBaseURL, neturl.PathEscape(email))
	...
	for _, usr := range r.Data {
		if usr == nil || usr.UserType() != userpb.UserType_USER_TYPE_LIGHTWEIGHT {
			continue
		}
...
func (m *manager) GetUser(ctx context.Context, uid *userpb.UserId, skipFetchingGroups bool) (*userpb.User, error) {
	...
	if err != nil {
		if uid == nil || uid.Type != userpb.UserType_USER_TYPE_LIGHTWEIGHT || !strings.Contains(uid.OpaqueId, "@") {
			return nil, err
		}
