...
import (
	...
	conversions "github.com/cernbox/reva-plugins/utils"
...
func (m *mgr) Unshare(ctx context.Context, ref *collaboration.ShareReference) error {
	...
	path, err := m.resolvePath(ctx, ref)
	if err != nil {
		return err
	}

	query, params, err = m.appendUidOwnerFilters(ctx, path, query, params)
	if err != nil {
		return err
	}
...
func (m *mgr) resolvePath(ctx context.Context, ref *collaboration.ShareReference) (string, error) {
	var path string
	var err error
	switch {
	case ref.GetId() != nil:
...
func (m *mgr) appendUidOwnerFilters(ctx context.Context, path, query string, params []interface{}) (string, []interface{}, error) {
	uidOwnersQuery, uidOwnersParams, err := m.uidOwnerFilters(ctx, path)
	if err != nil {
		return "", nil, err
	}
...
func (m *mgr) uidOwnerFilters(ctx context.Context, path string) (string, []interface{}, error) {
	user := ctxpkg.ContextMustGetUser(ctx)
	uid := conversions.FormatUserID(user.Id)
